...
use mc_consensus_api::{
    consensus_peer::GetLatestMsgResponse, consensus_peer_grpc::ConsensusPeerApiClient, consensus_common_grpc::BlockchainApiClient, consensus_common::LastBlockInfoResponse,  empty,
};
...
impl Crawler {
    ...
    fn crawl_node(&mut self, peer: &String) {
        ...
        if let (Some(consensus_client), Some(blockchain_client)) = Self::prepare_rpc(peer.clone()) {
			...
			let quorum_set = if let Some(rpc_reply) = Self::send_rpc_get_latest_msg(consensus_client) {
            ...
            // ===
            let (network_block_version, latest_block) = if let Some(rpc_reply) = Self::send_rpc_get_last_block_info(blockchain_client) {
				let network_block_version = rpc_reply.get_network_block_version();
				let index = rpc_reply.get_index();
				(network_block_version, index)
				} else {
                    warn!("Couldn't get network block version and latest block from {}.", peer);
                    (0,0)
                };
			let mut crawled = CrawledNode::new(peer.clone(), reachable, quorum_set, latest_block, network_block_version);
